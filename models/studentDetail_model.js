...
module.exports = function studentDetail(sid) {
    ...
    return new Promise((resolve, reject) => {
        db.query('SELECT stu.user_id as studentID, stu.name as studentName, stu.department, sum(s.score)/count(distinct s.unit_id) as average FROM units AS u JOIN scores AS s on u.id=s.unit_id JOIN students AS stu ON stu.user_id = s.user_id WHERE stu.user_id = ?',
            sid,
            function (err, rows) {
                if (err) {
                    console.log(err);
